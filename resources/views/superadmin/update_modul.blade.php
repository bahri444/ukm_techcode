...
@section('content')
    ...
    <div class="mx-auto mt-10 max-w-3xl bg-white">
        @foreach ($data as $val)
            @foreach ($val->joinToKategoriModul as $vals)
                <form method="post" action="/postupdatemodul" class="max-w-full mx-auto" enctype="multipart/form-data">
                    @csrf
                    <input name="modul_uuid" value="{{ $val->modul_uuid }}" type="hidden" id="small-input"
                    ...
                    <div>
                        ...
                        <select id="countries" name="kategori_uuid"
                            ...
                            <option value="{{ $val->kategori_uuid }}" selected>{{ $vals->nama_kategori }}</option>
                            @foreach ($kategori_modul as $rows)
                                <option value="{{ $rows->kategori_uuid }}">{{ $rows->nama_kategori }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <input name="judul" value="{{ $val->judul }}" type="text" id="small-input"
                            class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="materi"
                        ...
                        <textarea id="materi" rows="4" name="materi"
                            ...
                            placeholder="input your teks here...">{{ $val->materi }}</textarea>
                    </div>
                    <div>
                        <label for="kode"
                            class="block mb-2 mt-3 text-sm font-medium text-gray-900 dark:text-white">Kode</label>
                        <textarea id="kode" rows="4" name="kode"
                            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="input your kode here...">{{ $val->kode }}</textarea>
